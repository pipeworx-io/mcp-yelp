 ...
 *
 * BYO-only (fleet #2278): pass your own Yelp Fusion key via _apiKey. Pipeworx
 * fronts no platform key here — Fusion is $229/mo (#2275, open) — so a call
 * with no key is refused rather than downgraded. Auth is a Bearer header.
 */
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Yelp');
}
    ...
    inputSchema: {
      ...
      properties: {
        ...
        _apiKey: {
          type: 'string',
          description:
            'Required — your own Yelp Fusion key. Pipeworx fronts no platform key for Yelp (Fusion plan is $229/mo, #2275 open); every call is refused with none.',
        },
      },
    },
  },
...
async function getReviews(args: Record<string, unknown>, apiKey: string): Promise<unknown> {
  ...
  if (!id) {
    throw new Error(
      'Required argument "id" is missing. Pass a Yelp business id from search_businesses, e.g. get_reviews({id: "WavvLdfdP6g8aZTtbBQHTw"}). Yelp aliases (e.g. "garaje-san-francisco") no longer reliably resolve on /reviews — use search_businesses first to get a current encrypted id.'
    );
  }
  const params = new URLSearchParams();
  ...
  if (data && typeof data === 'object' && 'error' in data) {
    // Production analytics 2026-06-04: every get_reviews call was 404'ing,
    // including the docs example "garaje-san-francisco". Yelp's /reviews
    // endpoint no longer reliably accepts URL-slug aliases — only the
    // encrypted business id. Surface an actionable error so the agent
    // calls search_businesses first instead of looping on the same 404.
    const env = data as { error: number | string; message?: string };
    const status = env.error;
    if (status === 404 || status === '404') {
      const looksLikeAlias = id.includes('-') && !/^[A-Za-z0-9_-]{20,}$/.test(id);
      const hint = looksLikeAlias
        ? `Yelp returned 404 for "${id}". This looks like an alias/slug — Yelp's /reviews endpoint requires the encrypted business id, not the URL slug. Call search_businesses({term: "...", location: "..."}) first; each result's "id" field is the encrypted id to pass here.`
        : `Yelp returned 404 for "${id}". The business may not exist, may have been removed, or your Yelp Fusion key may not have review access. Verify the id by calling get_business({id: "${id}"}) first; if that also 404s the id is invalid.`;
      return { error: 404, message: hint, _hint_class: 'use_search_first' };
    }
    return data;
  }
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  if (!apiKey) {
    // Keyless refusal. Two things this MUST do (fleet #2271 — search_businesses
    // was selected 47 times in 30 days of ask_pipeworx traffic and every one
    // died here with nothing a caller could act on):
    //   1. Say "requires an API key" in the message — the quality classifier
    //      and the router's auth-retry key on that phrase, not on the shape.
    //   2. Name what to try INSTEAD, as `did_you_mean`, with the arguments
    //      already translated so the caller does not have to map term→query.
        ...
        ? [
            {
              tool: 'search_places',
              pack: 'foursquare',
              why: 'Keyless — Pipeworx supplies the key. Same shape (query + near). sort:"RATING" orders by Foursquare rating; the rating number is a Premium field and is present only where enabled (read rating_included / rating_note in its response).',
              args: { query: term ?? 'coffee', near: location ?? 'San Francisco, CA', sort: 'RATING', limit: 10 },
            },
            {
              tool: 'places',
              pack: 'serper',
              why: 'Google Maps results WITH rating and review count. Platform key is for paid Pipeworx accounts; anyone can BYO a serper.dev key via _apiKey.',
              args: { query: [term, location].filter(Boolean).join(' in ') || 'coffee in San Francisco' },
            },
            {
              tool: 'here_discover',
              pack: 'here',
              why: 'Keyless POI discovery (name, category, address, phone, distance). No ratings.',
              args: { query: term ?? 'coffee shops', near: location ?? 'San Francisco' },
            },
          ]
        : [
            {
              tool: 'get_place',
              pack: 'foursquare',
              why: 'Keyless place details by fsq_place_id (from search_places). Ratings/hours are Premium fields — see rating_included in its response.',
              args: { fsq_place_id: '<from search_places>' },
            },
          ];
    return {
      error: 'api_key_required',
      message:
        `Yelp ${name} requires an API key — your own Yelp Fusion key, passed as _apiKey (Pipeworx does not front a Yelp key: the plan is $229/mo). Create an app at https://docs.developer.yelp.com/docs/fusion-authentication. Without a key, try ${did_you_mean[0]!.tool} (${did_you_mean[0]!.pack}) — see did_you_mean.`,
      signup_hint: 'https://docs.developer.yelp.com/docs/fusion-authentication',
      did_you_mean,
      // Kept for callers that already key on this field (pre-#2271 shape).
      keyless_alternative:
        name === 'search_businesses'
          ? 'Keyless: search_places({query, near, sort:"RATING"}) on foursquare (rating-ordered; rating number only where Premium is enabled), or here_discover({query, near}) for plain discovery with no ratings.'
          : undefined,
    };
  }
